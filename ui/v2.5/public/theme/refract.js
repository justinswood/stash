...
(function () {
    ...
    function registerAccentPatch() {
        ...
        /* Native "Settings → Theme" tab. When Refract ships as the bundled
           default UI there is no plugin settings card to patch, so mount the
           same settings component into the tab's #refract-theme-settings-root
           element. The interval re-mounts when the tab is (re)opened — React
           destroys the element on navigate-away, so the data-attr guard resets
           with it. */
        var RD = PluginApi.ReactDOM;
        if (RD && RD.render) {
            setInterval(function () {
                var el = document.getElementById("refract-theme-settings-root");
                if (el && el.getAttribute("data-refract-mounted") !== "1") {
                    el.setAttribute("data-refract-mounted", "1");
                    RD.render(PluginApi.React.createElement(AccentSwatchPicker), el);
                }
            }, 300);
        }
    }
